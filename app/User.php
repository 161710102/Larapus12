...
class User extends Authenticatable
{
    ...
    public function generateVerificationToken() { 
        ...
        if (!$token) { 
            $token = str_random(40); $this->verification_token = 
            $token; $this->save(); 
        } 
        return $token; 
    }
    ...
    public function sendVerification() {
       ...
       Mail::send('auth.emails.verification', compact('user', 'token'), 
        function ($m) use ($user
) {
    ...
    public function borrow(Book $book) { 
        if ($book->stock < 1) { throw new BookException("Buku $book->title sedang tidak tersedia."); }
